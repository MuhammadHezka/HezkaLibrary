...
$dataPinjam = queryReadData("SELECT peminjaman.id_peminjaman, peminjaman.id_buku, buku.judul, peminjaman.nisn, member.nama, admin.nama_admin, peminjaman.tgl_peminjaman, peminjaman.tgl_pengembalian, peminjaman.status, buku.isi_buku
                              ...
                              WHERE peminjaman.nisn = '$akunMember' AND (peminjaman.status = 'hapus' OR peminjaman.status = 'Waktu habis')");
...
<html lang="en">
  ...
  <div class="p-4 mt-5">
    ...
    <div class="mt-5 alert alert-bgwarning" role="alert">Riwayat transaksi Peminjaman Buku Anda - <span class="fw-bold text-capitalize"><?php echo htmlentities($_SESSION["member"]["nama"]); ?></span></div>
    ...
    <table class="table table-striped table-hover">
     <thead class="text-center">
      <tr>
        ...
        <th class="bg-warning text-light">Judul Buku</th>
        <th class="bg-warning text-light">Nisn</th>
        <th class="bg-warning text-light">Nama</th>
        <th class="bg-warning text-light">Nama Admin</th>
        <th class="bg-warning text-light">Tanggal Peminjaman</th>
        <th class="bg-warning text-light">Tanggal Berakhir</th>
